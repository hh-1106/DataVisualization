...
class Morpher {
  ...
  void load2Points(String pathA, String pathB) {
    pairs.clear();
    String[] linesA = loadStrings(ptsPath + pathA);
    String[] linesB = loadStrings(ptsPath + pathB);
    
    for (int i = 3; i < 68; i++) {
      float[] valsA = float(split(linesA[i], " "));
      float[] valsB = float(split(linesB[i], " "));
      addPair(new PVector(valsA[0], valsA[1]), new PVector(valsB[0], valsB[1]));
    }
  }
}
